...
/**
 * Importazione fornitori da un'esportazione Danea.
 * Stesso motore dei clienti: il file dei soggetti ha la stessa struttura.
 * L'archivio di provenienza è obbligatorio e mai dedotto dal file.
 */
...
export function SupplierImportDialog({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!result.customers.length) {
        toast.error("Nel file non ho trovato fornitori da importare.");
        return;
      }
      setParsed(result);
      applyPreview(result);
  ...
  async function confirmImport() {
    ...
    if (!rows.length) {
      toast.error("Seleziona almeno un fornitore da importare.");
      return;
    }
    if (!archiveId) {
    ...
    for (const item of rows) {
      ...
      const payload = {
        ...
        ...(item.existingId ? { _supplier_record_id: item.existingId } : {}),
      ...
      const { error } = await supabase.rpc("manage_supplier_record", payload);
      if (error) {
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[90vh] w-[calc(100vw-1.5rem)] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Importa fornitori da Danea</DialogTitle>
          <DialogDescription>
        ...
        <div className="grid gap-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Obbligatorio: fornitori e codici restano separati per archivio. Lo stesso codice in due
            archivi resta una scheda distinta.
          </p>
        </div>

        <div className="grid gap-1.5">
          <Label htmlFor="supplier-import-file">File Danea</Label>
          <input
        ...
        {parsed && parsed.headers.length ? (
          <div className="space-y-2 rounded-lg border border-border p-3">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              in “Altri dati (Danea)” nella scheda del fornitore.
            </p>
        ...
        <DialogFooter>
          ...
          <Button disabled={busy || !preview.length || !archiveId} onClick={confirmImport}>
            {busy ? "Importazione…" : `Importa ${selected.size} fornitori`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
