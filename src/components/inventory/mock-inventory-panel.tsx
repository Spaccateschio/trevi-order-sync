...
function ProductTableRow({ product, value, confirmed, onChange, onConfirm }: { product: MockProduct; value: string; confirmed?: number; onChange: (value: string) => void; onConfirm: () => void }) {
...
function ProductMobileRow({ product, value, confirmed, onChange, onConfirm }: { product: MockProduct; value: string; confirmed?: number; onChange: (value: string) => void; onConfirm: () => void }) {
  const parsed = value ? parseLocalQuantity(value) : null;
  const effective = confirmed ?? parsed;
  const difference = effective === null || effective === undefined ? null : effective - product.calculated;
...
function CountSummary({ location, confirmed, onBack, onFinish }: { location?: MockLocation; confirmed: Record<string, number>; onBack: () => void; onFinish: () => void }) {
